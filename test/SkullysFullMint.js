    ...
    it("In public-sale, can mint Skullys (multiple) whether whitelisted or not", async function () {
        ...
        for (let w = 1; w < numWallets; w++) {
            ...
            for (let s = 0; s < skullysPerWallet; s++) {
                mintPromises[w-1].push(this.skullys.connect(wallets[w]).mintSkully({ value: ethers.utils.parseEther("50") }))
                mintCount++
            }
        }
        ...
        for (let s = 0; s < supplyLeft; s++) {
            await this.skullys.connect(wallets[1]).mintSkully({ value: ethers.utils.parseEther("50") })
            mintCount++
        }
        ...
        await expectRevert(
            this.skullys.connect(wallets[1]).mintSkully({ value: ethers.utils.parseEther("50") }),
            "Sold out"
        )
        ...
        expect(artBalAfterMint.sub(origArtBal)).to.equal(BigNumber.from(ethers.utils.parseEther("50000")))
