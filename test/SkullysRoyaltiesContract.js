...
describe("Full mint test harness for Skullys using auxiliary royalties contract", function () {
    ...
    before(async function () {
        ...
        this.SkullysPayments = await ethers.getContractFactory("SkullysPayments")
        this.Unpayable       = await ethers.getContractFactory("Unpayable")
    });
    ...
    beforeEach(async function () {
        ...
        this.unpayable = await this.Unpayable.connect(this.alice).deploy()
        await this.unpayable.deployed()

        this.payments = await this.SkullysPayments.connect(this.alice).deploy(this.skullys.address)
    ...
    it("In public-sale, can mint Skullys (multiple) whether whitelisted or not", async function () {
        ...
        for (let w = 1; w < numWallets; w++) {
            for (let s = 0; s < skullysPerWallet; s++) {
                ...
                tokenId++;
                if (tokenId == 69) {
                    // Unpayable contract owns 69, but this shouldn't break royalties
                    // 69's royalties should just go to Funeral
                    this.skullys.connect(wallets[w]).transferFrom(wallets[w].address, this.unpayable.address, tokenId);
                }
            }
        }
        ...
        await this.alice.sendTransaction({ to: this.payments.address, value: COST })
        let paymentsBal = await this.provider.getBalance(this.payments.address)
        expect(paymentsBal).to.equal(COST)

        // alice triggers the withrawAll function which distributes the contract balance back to the team, etc
        await this.payments.connect(this.alice).withdrawAll()
        paymentsBal = await this.provider.getBalance(this.payments.address)
        ...
        // Skully 69 was sent to an unpayable contract, its royalties should go to Funeral
        //expect(bal69).to.equal(BigNumber.from(ethers.utils.parseEther("3")))
        expect(bal69).to.equal(BigNumber.from(ethers.utils.parseEther("0")))
